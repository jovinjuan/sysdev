...
$total = '';
$grandtotal = '';
$tanggalpenjualan = date('Y-m-d'); // Default to today
$errors = [];
$success_message = '';

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $notajual = trim($_POST['notajual']);
    $namabarang = trim($_POST['namabarang']);
    $harga = trim($_POST['harga']);
    $jumlah = trim($_POST['jumlah']);
    // total and grandtotal might be calculated or entered, for now, let's assume they are entered
    $total = trim($_POST['total']);
    $grandtotal = trim($_POST['grandtotal']);
    $tanggalpenjualan = trim($_POST['tanggalpenjualan']);

    // Basic Validations
    if (empty($notajual)) $errors[] = "Nota Jual tidak boleh kosong.";
    if (empty($namabarang)) $errors[] = "Nama Barang tidak boleh kosong.";
    if (!is_numeric($harga) || $harga < 0) $errors[] = "Harga harus angka positif.";
    if (!is_numeric($jumlah) || $jumlah <= 0) $errors[] = "Jumlah harus angka positif lebih dari 0.";
    if (!is_numeric($total) || $total < 0) $errors[] = "Total harus angka positif.";
    if (!is_numeric($grandtotal) || $grandtotal < 0) $errors[] = "Grand Total harus angka positif.";
    if (empty($tanggalpenjualan)) $errors[] = "Tanggal Penjualan tidak boleh kosong.";
    // Add more specific validations as needed

    if (empty($errors)) {
        try {
            // kodenota, uangpembeli, kembalian are not handled in this simplified form
            // waktuinputjual will use default current_timestamp()
            $stmt = $conn->prepare("INSERT INTO penjualan (notajual, namabarang, harga, jumlah, total, grandtotal, tanggalpenjualan, kodenota) 
                                    VALUES (:notajual, :namabarang, :harga, :jumlah, :total, :grandtotal, :tanggalpenjualan, '')");
            $stmt->bindParam(':notajual', $notajual);
            $stmt->bindParam(':namabarang', $namabarang);
            $stmt->bindParam(':harga', $harga);
            $stmt->bindParam(':jumlah', $jumlah);
            $stmt->bindParam(':total', $total);
            $stmt->bindParam(':grandtotal', $grandtotal);
            $stmt->bindParam(':tanggalpenjualan', $tanggalpenjualan);
            $stmt->execute();
            $success_message = "Data penjualan berhasil ditambahkan!";
            // Clear form
            $notajual = $namabarang = $harga = $jumlah = $total = $grandtotal = '';
            $tanggalpenjualan = date('Y-m-d');
        } catch (PDOException $e) {
            $errors[] = "Gagal menambahkan data penjualan: " . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title>Tambah Penjualan - Sistem Gudang</title>
        ...
        <div class="form-container">
            ...
            <form method="POST" action="tambah_penjualan.php">
                ...
                <div class="d-grid gap-2">
                    <button type="submit" class="btn btn-custom">Tambah Penjualan</button>
